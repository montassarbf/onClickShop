...
const writeCache = (items: OrderItem[]) => {
  try { localStorage.setItem(CART_ITEMS_CACHE, JSON.stringify(items)); } catch {}
};
  ...
  const handleCheckout = async () => {
    try {
      await api.post("/orders", {
        items: orderItems.map(item => ({ product_id: item.product.id, quantity: item.quantity, price: item.price })),
      });
        ...
        {orderItems.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 flex flex-col gap-4">
              {orderItems.map(item => (
                <div key={item.id} className="bg-white shadow-md hover:shadow-xl transition duration-300 rounded-2xl p-4 flex items-center gap-4 border border-gray-100">
                  <img src={item.product.image_url || "https://ih1.redbubble.net/image.5103371828.7438/flat,750x,075,f-pad,750x1000,f8f8f8.jpg"} alt={item.product.name} className="w-20 h-20 rounded-xl object-cover shrink-0" />
                  ...
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    ...
                    <button onClick={() => handleRemove(item.id, item.product.id)} className="w-8 h-8 rounded-full flex items-center justify-center text-gray-300 hover:text-red-400 hover:bg-red-50 transition duration-200">
              ...
              <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6 mt-2">
                ...
                <div className="flex justify-between text-gray-500 text-sm mb-2"><span>Subtotal</span><span>${total.toFixed(2)}</span></div>
                <div className="flex justify-between text-gray-500 text-sm mb-4"><span>Shipping</span><span>${shipping.toFixed(2)}</span></div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6 sticky top-24">
                ...
                <div className="grid grid-cols-4 gap-2 mb-6">
                  {(["visa", "mastercard", "paypal", "amex"] as PaymentMethod[]).map((method) => (
                    <button key={method} type="button" onClick={() => setPaymentMethod(method)}
                      className={`rounded-xl p-1.5 flex items-center justify-center border-2 transition duration-200 ${paymentMethod === method ? "border-orange-400 bg-orange-50 shadow-sm" : "border-gray-100 hover:border-gray-300"}`}>
                ...
                {isCardMethod && (
                  <div className="flex flex-col gap-4">
                    <div className="relative w-full h-36 rounded-2xl overflow-hidden bg-gradient-to-br from-gray-800 to-gray-900 p-5 shadow-lg mb-2">
                      ...
                      <p className="text-white/80 text-sm font-mono tracking-widest">{cardNumber || "•••• •••• •••• ••••"}</p>
                    ...
                    <div>
                      ...
                      <input type="text" placeholder="1234 5678 9012 3456" value={cardNumber} onChange={(e) => setCardNumber(formatCardNumber(e.target.value))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-orange-400 transition bg-gray-50" />
                    </div>
                    <div>
                      <label className="text-xs text-gray-500 font-medium mb-1 block">Cardholder Name</label>
                      <input type="text" placeholder="John Doe" value={cardName} onChange={(e) => setCardName(e.target.value.toUpperCase())} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-orange-400 transition bg-gray-50" />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs text-gray-500 font-medium mb-1 block">Expiry Date</label>
                        <input type="text" placeholder="MM/YY" value={expiry} onChange={(e) => setExpiry(formatExpiry(e.target.value))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-orange-400 transition bg-gray-50" />
                      </div>
                      <div>
                        <label className="text-xs text-gray-500 font-medium mb-1 block">CVV</label>
                        <div className="relative">
                          <input type={showCvv ? "text" : "password"} placeholder="•••" maxLength={4} value={cvv} onChange={(e) => setCvv(e.target.value.replace(/\D/g, "").slice(0, 4))} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-orange-400 transition bg-gray-50 pr-9" />
                          <button type="button" onClick={() => setShowCvv(!showCvv)} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                ...
                {paymentMethod === "paypal" && (
                  <div className="flex flex-col gap-4">
                    ...
                    <div>
                      ...
                      <input type="email" placeholder="you@paypal.com" value={payEmail} onChange={(e) => setPayEmail(e.target.value)} className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-orange-400 transition bg-gray-50" />
                    </div>
                  </div>
                )}
                ...
                <button onClick={handleCheckout} className="w-full py-3 rounded-xl bg-orange-500 hover:bg-orange-600 text-white font-bold text-base transition duration-200 shadow-md shadow-orange-200 flex items-center justify-center gap-2">
                ...
                <p className="text-center text-xs text-gray-400 mt-3">This is a demo — no real payment will be processed.</p>
              </div>
            </div>
          </div>
        )}
