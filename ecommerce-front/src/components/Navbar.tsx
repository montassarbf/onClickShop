...
/** Clears all localStorage keys related to the current user account. */
const clearAllUserCache = () => {
  ...
  // Remove all profile cache entries (keyed by token suffix).
...
const Navbar: React.FC = () => {
  ...
  const [apiCartTotal, setApiCartTotal] = useState<number>(
    () => Number(localStorage.getItem(CART_COUNT_CACHE_KEY) || "0")
  );

  // Fetch the real cart count from the API for logged-in users.
  const fetchApiCartCount = useCallback(async () => {
    ...
    try {
      ...
      localStorage.setItem(CART_COUNT_CACHE_KEY, String(total));
  ...
  // Fetch profile image once on mount if not already cached.
  useEffect(() => {
    const token = localStorage.getItem("token");
  ...
  // Refresh cart count whenever the cart or route changes.
  useEffect(() => {
    fetchApiCartCount();
  }, [fetchApiCartCount, cartCount, location.pathname, isLoggedIn]);

  // Highlight the active section based on scroll position (home page only).
  useEffect(() => {
    const handleScroll = () => {
  ...
  // Use the API cart count for logged-in users, local count for guests.
  ...
  const avatarUrl = profileImage || DEFAULT_AVATAR;
  ...
  return (
    <div className="fixed top-0 left-0 right-0 navbar bg-white text-gray-800 shadow-sm pt-3 pb-3 px-3 lg:pr-10 w-full max-w-full mx-auto rounded-b-xl z-50 border-b border-gray-100/80">
      ...
      {/* ── Left: burger (mobile) + desktop nav links ── */}
      <div className="navbar-start flex-1">

        {/* Mobile burger menu */}
        <div className="dropdown lg:hidden">
          <div tabIndex={0} role="button" className="btn btn-ghost btn-circle btn-sm">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="black" aria-label="Menu">
      ...
      <div className="navbar-center">
        ...
        >
          <img src={logo} alt="Onclick Shop" className="w-16 md:w-24" />
        </button>
      </div>
      ...
      <div className="navbar-end flex-1 justify-end flex items-center gap-1">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" version="1.0" width={18} height={18} viewBox="0 0 512.000000 512.000000" preserveAspectRatio="xMidYMid meet" className="text-gray-800" aria-hidden="true">
          ...
          {displayCartCount > 0 && (
            <span className="absolute -right-0.5 -top-0.5 min-h-[1rem] min-w-[1rem] px-0.5 flex items-center justify-center rounded-full bg-orange-500 text-white text-[9px] font-semibold leading-none tabular-nums">
        ...
        <div className="dropdown dropdown-end">
          ...
          >
            <div className="indicator">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-gray-800" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
              </svg>
              {unreadCount > 0 && (
                <span className="badge badge-xs badge-primary indicator-item border-0 bg-orange-500 text-white">
                  {unreadCount}
                </span>
              )}
            </div>
          </button>
          <ul tabIndex={0} className="dropdown-content menu bg-white rounded-box z-[60] w-64 p-0 shadow-lg border border-gray-100 mt-2">
            ...
            <li className="border-t border-gray-100">
              <button
                type="button"
                className="text-orange-500 text-sm py-2 justify-center"
        ...
        {/* Avatar menu (logged in) or Login/Sign Up buttons (guest) */}
        {isLoggedIn ? (
          <div className="dropdown dropdown-end">
            <div
              tabIndex={0}
              role="button"
              className="btn btn-ghost btn-circle btn-sm avatar ring-2 ring-orange-100 hover:ring-orange-300"
            >
            ...
            <ul tabIndex={0} className="menu menu-sm dropdown-content bg-white rounded-box z-[60] mt-3 w-52 p-2 shadow-lg border border-gray-100">
              <li className="p-2">
                <button type="button" onClick={() => navigate("/profile")} className="flex items-center gap-2">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#3B3B3B" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          ...
          <div className="flex gap-1 sm:gap-2 items-center">
            ...
            <button
              type="button"
              className="btn btn-xs sm:btn-sm rounded-full bg-orange-400 hover:bg-orange-500 text-white border-0 px-3 sm:px-5 min-h-0 h-8 sm:h-9 text-xs sm:text-sm shadow-sm"
